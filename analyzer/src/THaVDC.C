				  ...
				  const char* prefix )
{
  ...
  //I think this is actually the line from the DB files we're parsing here
  ...
  while( ist >> word ) {//parse a line of the string
    
    if( !findnext ) {//now, we will parse the rest of this line
            
      assert( cur != matrix_map.end() );//check to see if we have elements left
	...
	//check to make sure this exponent is ok
	...
	//add this exponent to the rest of this matrix
    ...
    if( findnext ) { //look at the first char of the line
      
      //this line is one letter, which defines which 'matrix' this element 
      // belongs to
      cur = matrix_map.find(word);
            
      if( cur == matrix_map.end() ) { //matrix element not found
	// Error( Here(here,prefix), "Unknown matrix element type %s. Fix database.",
	// 	 word.c_str() );
	// return THaAnalysisObject::kInitError;
	continue;
      }
      //we're starting a new matrix element
      ME.clear();
      
      findnext   = false; //don't look at the next line (for the moment)
      
      findpowers = true;  //look for the three powers which define this element
            
      //number of exponents to parse (3, at least for APEX)
      powers_left = cur->second.npow;
      
      //'w' is the name of the Matrix to which this element belongs
      w = word;
    }//if( findnext ) { 
  } // while(word)
...
void THaVDC::CalcFocalPlaneCoords( THaTrack* track )
{
  ...
  Double_t r_phi 
    = (track->GetDPhi() - fFPMatrixElems[P000].v) /
...
#if DEBUG_OPTICS 
  cout << "THaVDC::CalcTargetCoords()" << endl; 
  cout << TString::Format("tg_y  old=%0.4e", y)     << endl; 
  cout << TString::Format("tg_th old=%0.4e", theta) << endl; 
			  
       
  cout << TString::Format("tg_ph old=%0.4e", phi)   << endl; 
	
  cout << TString::Format("tg_dp old=%0.4e", dp)    << endl; 
#endif
			       ...
			       const Double_t powers[][5])
{
...
#if DEBUG_OPTICS
  cout << "THaVDC::CalcTargetVar() - matrix n.=" << matrix.size() << endl; 
  
  cout << TString::Format("intputs = [x=%0.3e, y=%0.3e, th=%0.3e, ph=%0.3e]", 
			  powers[1][0], 
			  powers[1][2], 
			  powers[1][1], 
			  powers[1][3] ) << endl;  
  ...
  for( vector<THaMatrixElement>::const_iterator it=matrix.begin();
    ...
    cout << TString::Format("elem-(%2i), exp=[%i,%i,%i,%i], poly=[%0.3e], ",
			    eNum, 
			    it->pw[0], it->pw[1], it->pw[2], it->pw[3], 
			    it->v ); 
    eNum++; 
#endif 
...
#if DEBUG_OPTICS       
      cout << TString::Format("M=[%0.3e]", v) << endl; 
#endif 
...
#if DEBUG_OPTICS 
  cout << TString::Format("final val = %0.3e", retval) << endl; 
#endif 
