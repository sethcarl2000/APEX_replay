...
/// @brief Form 'TapexVDCHitGroup's from groups of VDC hits
...
ROOT::RVec<TapexVDCHitGroup> group_vdc_hits ( 
                                            ...
                                            ) 
{
    ...
    ROOT::RVec<TapexVDCHitGroup> group_vec{}; 

    //cBeg->push_back((int)wire[0]);   
    TapexVDCHitGroup group(p); 
    ...
    for (int h=0; h<n_hits; h++) { 
      ...
      TapexVDCHit hit( p, (int)std::round(h_wire[h]), h_rawtime[h], evt );  
