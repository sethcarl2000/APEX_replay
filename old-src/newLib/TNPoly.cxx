...
//_____________________________________________________________________________
vector<double> TNPoly::Gradient(const vector<double> *const coeff, const vector<double> X) const
{
  if (coeff->size() != Get_nElems()) { 
    Error("TNPoly::Eval()",
	  "Wrong number of coeffs. given; expected %i, recieved %i.",
  ...
  ROOT::VecOps::RVec<double> grad(fnDoF,0.); 
  ...
  for (TNPolyElem *elem : fElems) {
    ...
    ROOT::VecOps::RVec<double> grad_elem; grad_elem.reserve(fnDoF);   
