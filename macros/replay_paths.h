...
namespace replay_paths
{
  ...
  const std::vector<replay_segment> segments_laptop {
    {3875, 3899,   129801,  true, "/mnt/d/jlab/APEX/full-replay-may8/replay-3875-3899.root"},
    {4200, 4224, 38198197,  true, "/mnt/d/jlab/APEX/full-replay-may8/replay-4200-4224.root"},
    {4225, 4249, 37966339,  true, "/mnt/d/jlab/APEX/full-replay-may8/replay-4225-4249.root"},
    {4250, 4274, 37318859,  true, "/mnt/d/jlab/APEX/full-replay-may8/replay-4250-4274.root"}
  }; 

  const std::vector<replay_segment> segment_laptop_small {
    {3875, 3899,   129801,  true, "/mnt/d/jlab/APEX/full-replay-may8/replay-3875-3899.root"}
  }; 

  const std::vector<replay_segment> segments_ifarm {
  ...
  /// @brief Fetch a list of replay_segment-s
  /// @param identifier identifier of the segment. see implementation for valid options. 
  /// @return list of replay segment information corresponding to 'identifier', or empty list if 'identifier' is an invalid option. 
  std::vector<replay_segment> get_segment(std::string identifier) {
    //list of all defined segments 
    static const std::map<std::string, const std::vector<replay_segment>*> segment_list{
      {"ifarm-all",         &segments_ifarm},
      {"laptop-all",        &segments_laptop},
      {"laptop-one-small",  &segment_laptop_small}
    };
    //
    const auto it = segment_list.find(identifier); 
    
    // in this case, we have found a valid segment. return a copy of the list from above. 
    if (it != segment_list.end()) { return std::vector<replay_segment>{*it->second}; } 
    
    //otherwise, we faild to find a list corresponding to 'identifier'. 
    //we will throw a 'logic_error' exception, and return an empty list. 
    std::ostringstream oss; 
    oss << "in <replay_paths"<<__func__<<">: invalid segment list identifier: '"<<identifier<<"'. Valid options are: ";
    for (auto itt = segment_list.begin(); itt != segment_list.end(); itt++) oss << itt->first << " "; 
