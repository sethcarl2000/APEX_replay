...
void execute_specific_run(const int run_number, const std::string& path_task_list, const std::string& output_directory)
{
    ...
    const auto all_rawfiles = utils::get_task_rawfile_list(path_task_list, task_id); 

    if (all_rawfiles.empty()) {

        Error(__func__, "No rawfiles find assigned to task %u. task list file: %s", task_id, path_task_list.c_str()); 
        return;
    }
    
    Info(__func__, "Task id %u has %zi rawfiles to process:", task_id, all_rawfiles.size()); 

    // get all rawfiles for this particular run 
    std::vector<utils::Rawfile> rawfile_list;
    for (const auto& rawfile : all_rawfiles) {
      if (rawfile.run_number == run_number) rawfile_list.emplace_back(rawfile);
    }
